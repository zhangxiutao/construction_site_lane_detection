...
def full_search(binary_warped, visualization=False):
    ...
    if visualization:
        ...
        cv2.circle(out_img, (int(left_fitx[binary_warped.shape[0]-1]),binary_warped.shape[0]-1), 10, (255,255,255), 2)
        cv2.circle(out_img, (int(right_fitx[binary_warped.shape[0]-1]),binary_warped.shape[0]-1), 10, (255,255,255), 2)
        cv2.circle(out_img, (int(left_fitx[0]),0), 10, (255,255,255), 2)
        cv2.circle(out_img, (int(right_fitx[0]),0), 10, (255,255,255), 2)
...
def off_center(left, mid, right):
    ...
    if a >= b:  # driving right off plus
        offset = a / width * LANEWIDTH - LANEWIDTH /2.0
    else:       # driving left off minus
...
def compute_car_offcenter(ploty, left_fitx, right_fitx, undist):
    ...
    bottom_l = left_fitx[height-1] #为什么左边用最底的，右边用最上的？
...
def process_frame(img, visualization=False):
    ...
    img_binary = cv2.Canny(img_undist,50,100)
